...
CF:SetScript(
	...
	function(self, event)
		...
		if
			(IsAddOnLoaded("Shadowed Unit Frames") or IsAddOnLoaded("PitBull Unit Frames 4.0") or
...
local function OnEvent(self, event, addon)
	...
	if
		(IsAddOnLoaded("Blizzard_TalentUI") and IsAddOnLoaded("Blizzard_TimeManager") and
			IsAddOnLoaded("Blizzard_TradeSkillUI"))
	 then
		self:UnregisterEvent(event)
	end
end
